...
export function ClaudeIcon({ size = 16, className }: BrandIconProps) {
  return (
...
const TILES = {
  claude: "brand-tile-claude",
  codex: "brand-tile-codex",
  opencode: "brand-tile-opencode",
} as const;
...
export function BrandTile({ kind, size = 18 }: { kind: BrandKind; size?: number }) {
  ...
  return (
    <span className={`brand-tile ${TILES[kind]}`} aria-hidden="true">
      <Icon size={size - 6} />
    </span>
  );
}
